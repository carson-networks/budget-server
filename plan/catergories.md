...
| **Expense or income** | Direction of the category | Enum: expense vs income |
...
- **Migration order**: Create the `categories` table (with its self-referential `parent_id` FK) first. Then add `ALTER TABLE transactions ADD CONSTRAINT ... FOREIGN KEY (category_id) REFERENCES categories(id)`. If `transactions` was created before `categories` existed, the `category_id` column already exists; the new migration only adds the FK constraint.
...
- **Expense/income and transaction amount**: Today, transaction amount sign might already imply direction. Consider whether category expense/income should match transaction sign, or be advisory for reporting only; document the rule and enforce in validation if desired.
